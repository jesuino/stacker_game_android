...
class StackerGame extends Game {
    ...
    StackerGame(float w, float h, GraphicsContext _gc) {
        ...
        EventHandler setMousePressed = new EventHandler() {
        ...
        _gc.getCanvas().setOnMousePressed(setMousePressed);
        _gc.getCanvas().setOnTouchPressed(setMousePressed);
    ...
    public void update() {
        ...
        if (mousePressed) {
            checkStack();
            if (MOVING_LINE == 0) {                
                level.set(constrain(level.get() + 1, 0, MAX_LEVEL));
                updateMatrix();
            } else {
                MOVING_LINE--;
